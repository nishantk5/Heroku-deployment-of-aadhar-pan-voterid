...
def Voter_IDFRONT(img):
    ...
    res = {}
    name_list = []
    name_list= get_name5(tt)
    res['Name1'] = name_list[0]
    res['Name2'] = name_list[1]
    res['Name3'] = name_list[2]
...
def get_name5(tt):
    ...
    for i in tt:
        if i.startswith("Name"):
            index = tt.index(i)
            if tt[index + 1] == ":":
                s = s+tt[index+2].strip()
                s1 = s1+tt[index + 2].strip() + " " + tt[index + 3].strip()
                i = tt[index + 4]
                if sum(1 for c in i if c.isupper()) == 1:  # Count the no of Capital Letters:
                    s2 = s1 + " " + tt[index + 4].strip()
                break
            else:
                s = s+tt[index+1].strip()
                s1 = s1+tt[index + 1].strip() + " " + tt[index + 2].strip()
                i = tt[index + 3]
                if sum(1 for c in i if c.isupper()) == 1:  # Count the no of Capital Letters:
                    s2 = s1+ " " + tt[index + 3].strip()
